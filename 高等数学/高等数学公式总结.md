...
## $\int_{L_{AB}} P(x,y)dx + Q(x,y)dy = \int_{\alpha}^{\beta}\{P[\varphi(t), \varphi(t)]\varphi'(t) + Q[\varphi(t),\varphi(t)]\varphi'(t)\}dt$
...
* $f(x) = \frac{1}{1 - x} = \sum_{n=0}^{\infty}x^n$​
* $f(x) = ln(1 + x) = \sum_{n=0}^{\infty}\frac{(-1)^{n-1}}{n}x^{n}$
