...
* **存在未定义的情况，利用特殊极限和性质**
  ...
  **特殊极限：$\lim_{n\rightarrow0}(\frac{\sin{x}}{x}) = 1$​​，包含乘三角函数的极限， 三角函数是定界函数，所以有界函数乘以无穷小还是无穷小**

* **可以令 $y=kx 或 y=kx^2$​ 带入原函数，计算极限看是否相等，不相等则不满足定理，不存在极限**
...
* X型区域的计算(先对 y 进行积分，再对 x 进行积分，需要确定 y 的 上下界)

  $\int\int_Df(x,y)dxdy = \int_a^b dx \int_{\varphi_1(x)}^{\varphi_2(x)}f(x,y)dy$

* Y型区域的计算

  $\int\int_Df(x,y)dxdy = \int_c^d dy \int_{\varphi_1(y)}^{\varphi_2(y)}f(x,y)dx$
